...
export default function Profile() {
	...
	const handleSubmit = (e) => {
		...
		if (form.checkValidity() === true) {
			...
			if (!isDisabled && !toReset) {
				setValidated(false);
				dispatch(updateUserInfo({ data: value, history }));
			}

			if (!isDisabled && toReset) {
				if (isConfirmed) {
					setValidated(false);
					dispatch(changeUserPassword({ data: value, history }));
				} else {
	...
	const changeAvatar = (e) => {
		...
		dispatch(updateUserAvatar({ data: formData, type: "multipart/form-data", history }));
		setDisable(false);
		setToReset(false)
	}
